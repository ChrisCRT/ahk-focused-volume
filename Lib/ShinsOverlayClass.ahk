/**
 * Forked from {@link https://github.com/Spawnova/ShinsOverlayClass|Spawnova/ShinsOverlayClass}
 * @date 2026/04/10
 * @version 1.0.1
 * Added font weight options to text, and new function DrawArc
 */
...
class ShinsOverlayClass {
    ...
    __New(x_orTitle := 0, y_orClient := 1, width_orForeground := 1, height := 0, alwaysOnTop := 1, vsync := 0,
        clickThrough := 1, taskBarIcon := 0, guiID := 0) {
        ...
        this.gui := Gui("-DPIScale -Caption +E0x80000" (clickthrough ? " +E0x20" : "") (alwaysontop ? " +Alwaysontop" :
            "") (!taskBarIcon ? " +toolwindow" : ""), this.guiID)
        ...
        margins := Buffer(16, 0)
        NumPut("int", -1, margins, 0), NumPut("int", -1, margins, 4), NumPut("int", -1, margins, 8), NumPut("int", -1,
            margins, 12)
        ext := DllCall("dwmapi\DwmExtendFrameIntoClientArea", "Uptr", hwnd, "ptr", margins, "uint")
        if (ext != 0) {
            this.Err("Problem with DwmExtendFrameIntoClientArea",
                "overlay will not function`n`nReloading the script usually fixes this`n`nError: " DllCall(
                    "GetLastError", "uint") " / " ext)
            return
        }
        ...
        if (DllCall(this.vTable(this.factory, 11), "ptr", this.factory, "ptr", this.tBufferPtr, "ptr", 0, "uint", 0,
        "ptr*", &pOut) != 0) {
        ...
        if (DllCall(this.vTable(this.factory, 11), "ptr", this.factory, "ptr", this.tBufferPtr, "ptr", 0, "uint", 0,
        "ptr*", &pOut) != 0) {
            this.Err("Problem creating rounded stroke", "overlay will not function`n`nError: " DllCall("GetLastError",
                "uint"))
            return
        }
        ...
        if (DllCall(this.vTable(this.factory, 14), "Ptr", this.factory, "Ptr", this.rtPtr, "ptr", this.hrtPtr, "Ptr*", &
        pOut) != 0) {
            this.Err("Problem creating renderTarget", "overlay will not function`n`nError: " DllCall("GetLastError",
                "uint"))
            return
        }
        ...
        this.SetIdentity(4)
        if (DllCall(this.vTable(this.renderTarget, 8), "Ptr", this.renderTarget, "Ptr", this.colPtr, "Ptr", this.matrixPtr,
        "Ptr*", &pOut) != 0) {
        ...
        if (DllCall("dwrite\DWriteCreateFactory", "uint", 0, "Ptr", clsidwFactory, "Ptr*", &pOut) != 0) {
            this.Err("Problem creating writeFactory", "overlay will not function`n`nError: " DllCall("GetLastError",
                "uint"))
            return
        }
        this.wFactory := pOut
    ...
    AttachToWindow(title, AttachToClientArea := 0, foreground := 1, setOwner := 0) {
        ...
        if (!DllCall("GetWindowRect", "Uptr", this.attachHWND, "ptr", this.tBufferPtr)) {
            this.Err("AttachToWindow: Error", "Problem getting window rect, is window minimized?`n`nError: " DllCall(
                "GetLastError", "uint"))
            return 0
        }
    ...
    BeginDraw() {
        local pOut := 0
        if (this.attachHWND) {
            if (!DllCall("GetWindowRect", "Uptr", this.attachHWND, "ptr", this.tBufferPtr) or (this.attachForeground and
                DllCall("GetForegroundWindow", "cdecl Ptr") != this.attachHWND)) {
                if (this.drawing) {
                    this.Clear()
                    this.drawing := 0
    ...
    DrawImage(image, dstX, dstY, dstW := 0, dstH := 0, srcX := 0, srcY := 0, srcW := 0, srcH := 0, alpha := 1,
        drawCentered := 0, rotation := 0, rotOffX := 0, rotOffY := 0) {
        ...
        if (rotation != 0) {
            if (this.bits) {
                ...
                DllCall("d2d1\D2D1MakeRotateMatrix", "float", rotation, "float", dstX + (drawCentered ? 0 : dstW / 2),
                "float", dstY + (drawCentered ? 0 : dstH / 2), "ptr", this.matrixPtr)
            }
            DllCall(this._RMatrix, "ptr", this.renderTarget, "ptr", this.matrixPtr)
            DllCall(this._DrawImage, "ptr", this.renderTarget, "ptr", i["p"], "ptr", this.rect1Ptr, "float", alpha,
                "uint", this.interpolationMode, "ptr", this.rect2Ptr)
            this.SetIdentity()
            DllCall(this._RMatrix, "ptr", this.renderTarget, "ptr", this.matrixPtr)
        } else {
            DllCall(this._DrawImage, "ptr", this.renderTarget, "ptr", i["p"], "ptr", this.rect1Ptr, "float", alpha,
    ...
    DrawText(text, x, y, size := 18, color := 0xFFFFFFFF, fontName := "Arial", extraOptions := "") {
        ...
        DllCall(this.vTable(p, 3), "ptr", p, "uint", (InStr(extraOptions, "aRight") ? 1 : InStr(extraOptions, "aCenter"
        ) ? 2 : 0))
        ...
        DllCall(this._DrawText, "ptr", this.renderTarget, "wstr", text, "uint", strlen(text), "ptr", p, "ptr", bf,
        "ptr", this.brush, "uint", 0, "uint", 0)
    }
    ...
    DrawEllipse(x, y, w, h, color, thickness := 1) {
        ...
        DllCall(this._DrawEllipse, "Ptr", this.renderTarget, "Ptr", bf, "ptr", this.brush, "float", thickness, "ptr",
            this.stroke)
    }
    ...
    ;startAngle         :   starting angle (degrees, same as your old code: -90 = top)
    ...
    DrawArc(x, y, radius, startAngle, sweepAngle, color, thickness := 1) {
        this.SetBrushColor(color)
        rad := 3.14159265 / 180
        sx := x + Cos(startAngle * rad) * radius
        sy := y + Sin(startAngle * rad) * radius
        ex := x + Cos((startAngle + sweepAngle) * rad) * radius
        ey := y + Sin((startAngle + sweepAngle) * rad) * radius
        pGeom := 0
        sink := 0
        ...
        if (this.bits) {
            bf := Buffer(64)
            NumPut("float", sx, bf, 0)
            NumPut("float", sy, bf, 4)
            DllCall(this.vTable(sink, 5), "ptr", sink, "double", NumGet(bf, 0, "double"), "uint", 1)
        } else {
            DllCall(this.vTable(sink, 5), "ptr", sink, "float", sx, "float", sy, "uint", 1)
        }

        arc := Buffer(64, 0)
        NumPut("float", ex, arc, 0)
        NumPut("float", ey, arc, 4)
        NumPut("float", radius, arc, 8)
        NumPut("float", radius, arc, 12)
        NumPut("float", 0, arc, 16)
        NumPut("uint", (sweepAngle >= 0 ? 1 : 0), arc, 20) ; direction
        NumPut("uint", (Abs(sweepAngle) > 180 ? 1 : 0), arc, 24)
        DllCall(this.vTable(sink, 14), "ptr", sink, "ptr", arc)
        DllCall(this.vTable(sink, 8), "ptr", sink, "uint", 0)
        DllCall(this.vTable(sink, 9), "ptr", sink)
        result := DllCall(this.vTable(this.renderTarget, 22),
    ...
    DrawRectangle(x, y, w, h, color, thickness := 1) {
        ...
        NumPut("float", x + w, bf, 8)
        NumPut("float", y + h, bf, 12)
        DllCall(this._DrawRectangle, "Ptr", this.renderTarget, "Ptr", bf, "ptr", this.brush, "float", thickness, "ptr",
            this.stroke)
    }
    ...
    DrawRoundedRectangle(x, y, w, h, radiusX, radiusY, color, thickness := 1) {
        ...
        DllCall(this._DrawRoundedRectangle, "Ptr", this.renderTarget, "Ptr", bf, "ptr", this.brush, "float", thickness,
            "ptr", this.stroke)
    }
    ...
    DrawLine(x1, y1, x2, y2, color := 0xFFFFFFFF, thickness := 1, rounded := 0) {
        ...
        if (this.bits) {
            ...
            DllCall(this._DrawLine, "Ptr", this.renderTarget, "Double", NumGet(bf, 0, "double"), "Double", NumGet(bf, 8,
                "double"), "ptr", this.brush, "float", thickness, "ptr", (rounded ? this.strokeRounded : this.stroke))
        } else {
            DllCall(this._DrawLine, "Ptr", this.renderTarget, "float", x1, "float", y1, "float", x2, "float", y2, "ptr",
                this.brush, "float", thickness, "ptr", (rounded ? this.strokeRounded : this.stroke))
        }
    ...
    DrawLines(points, color, connect := 0, thickness := 1, rounded := 0) {
        ...
        if (this.bits) {
            ...
            loop points.length - 1 {
                NumPut("float", lx, bf, 0), NumPut("float", ly, bf, 4), NumPut("float", lx := points[a_index + 1][1],
                    bf, 8), NumPut("float", ly := points[a_index + 1][2], bf, 12)
                DllCall(this._DrawLine, "Ptr", this.renderTarget, "Double", NumGet(bf, 0, "double"), "Double", NumGet(
                    bf, 8, "double"), "ptr", this.brush, "float", thickness, "ptr", (rounded ? this.strokeRounded :
                        this.stroke))
            }
            if (connect) {
                NumPut("float", sx, bf, 0), NumPut("float", sy, bf, 4), NumPut("float", lx, bf, 8), NumPut("float", ly,
                    bf, 12)
                DllCall(this._DrawLine, "Ptr", this.renderTarget, "Double", NumGet(bf, 0, "double"), "Double", NumGet(
                    bf, 8, "double"), "ptr", this.brush, "float", thickness, "ptr", (rounded ? this.strokeRounded :
                        this.stroke))
            ...
            loop points.length - 1 {
                ...
                DllCall(this._DrawLine, "Ptr", this.renderTarget, "float", x1, "float", y1, "float", x2, "float", y2,
                    "ptr", this.brush, "float", thickness, "ptr", (rounded ? this.strokeRounded : this.stroke))
            }
            if (connect)
                DllCall(this._DrawLine, "Ptr", this.renderTarget, "float", sx, "float", sy, "float", lx, "float", ly,
                    "ptr", this.brush, "float", thickness, "ptr", (rounded ? this.strokeRounded : this.stroke))
        }
        return 1
    }
        ...
        if (DllCall(this.vTable(this.factory, 10), "Ptr", this.factory, "Ptr*", &pGeom) = 0) {
            if (DllCall(this.vTable(pGeom, 17), "Ptr", pGeom, "Ptr*", &sink) = 0) {
                ...
                if (this.bits) {
                    ...
                    DllCall(this.vTable(sink, 5), "ptr", sink, "float", points[1][1] + xOffset, "float", points[1][2] +
                    yOffset, "uint", 1)
                    loop points.length - 1
                        DllCall(this.vTable(sink, 10), "ptr", sink, "float", points[a_index + 1][1] + xOffset, "float",
                        points[a_index + 1][2] + yOffset)
                    DllCall(this.vTable(sink, 8), "ptr", sink, "uint", 1)
                    DllCall(this.vTable(sink, 9), "ptr", sink)
                }

                if (DllCall(this.vTable(this.renderTarget, 22), "Ptr", this.renderTarget, "Ptr", pGeom, "ptr", this.brush,
                "float", thickness, "ptr", (rounded ? this.strokeRounded : this.stroke)) = 0) {
                    DllCall(this.vTable(sink, 2), "ptr", sink)
                    DllCall(this.vTable(pGeom, 2), "Ptr", pGeom)
                    return 1
                }
    ...
    FillPolygon(points, color, xoffset := 0, yoffset := 0) {
        ...
        if (DllCall(this.vTable(this.factory, 10), "Ptr", this.factory, "Ptr*", &pGeom) = 0) {
            if (DllCall(this.vTable(pGeom, 17), "Ptr", pGeom, "Ptr*", &sink) = 0) {
                ...
                if (DllCall(this.vTable(this.renderTarget, 23), "Ptr", this.renderTarget, "Ptr", pGeom, "ptr", this.brush,
                "ptr", 0) = 0) {
                    DllCall(this.vTable(sink, 2), "ptr", sink)
                    DllCall(this.vTable(pGeom, 2), "Ptr", pGeom)
                    return 1
    ...
    CacheImage(image) {
        ...
        if (this.bits) {
            ...
            NumPut("uint", w, bf, 0)
            NumPut("uint", h, bf, 4)
            if (v := DllCall(this.vTable(this.renderTarget, 4), "ptr", this.renderTarget, "int64", NumGet(bf, 0,
                "int64"), "ptr", p, "uint", 4 * w, "ptr", props, "Ptr*", &bitmap) != 0) {
                this.Err("Problem creating D2D bitmap for image '" image "'")
                return 0
            }
        } else {
            if (v := DllCall(this.vTable(this.renderTarget, 4), "ptr", this.renderTarget, "uint", w, "uint", h, "ptr",
            p, "uint", 4 * w, "ptr", props, "Ptr*", &bitmap) != 0) {
                this.Err("Problem creating D2D bitmap for image '" image "'")
                return 0
            }
        }
    ...
    CacheFont(name, size, weight) {
        local textFormat := 0
        if (DllCall(this.vTable(this.wFactory, 15), "ptr", this.wFactory, "wstr", name, "ptr", 0, "uint", weight,
        "uint",
        0, "uint", 5, "float", size, "wstr", "en-us", "Ptr*", &textFormat) != 0) {
            this.Err("Unable to create font: " name " (size: " size ", weight: " weight ")",
                "Try a different font or check to see if " name " is a valid font!"
            )
            return 0
        }
    ...
    Mcode(str) {
        ...
        if (s.length != 2)
            return
        if (!DllCall("crypt32\CryptStringToBinary", "str", s[this.bits + 1], "uint", 0, "uint", 1, "ptr", 0, "uint*", &
            pp, "ptr", 0, "ptr", 0))
            return
        p := DllCall("GlobalAlloc", "uint", 0, "ptr", pp, "ptr")
        if (this.bits)
            DllCall("VirtualProtect", "ptr", p, "ptr", pp, "uint", 0x40, "uint*", &op)
        if (DllCall("crypt32\CryptStringToBinary", "str", s[this.bits + 1], "uint", 0, "uint", 1, "ptr", p, "uint*", &
            pp, "ptr", 0, "ptr", 0))
            return p
